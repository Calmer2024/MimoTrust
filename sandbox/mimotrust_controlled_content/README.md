...
- 优先从云端 `/v1/feed` 加载五类内容，失败时回退到打包的三视频 registry；
- 校验 Manifest 1.0、Provider、内容身份和分析资源哈希；
- 支持视频、音频、文章、图文和图片画廊的展示与当前状态快照；
...
- 打开评论/转发面板时冻结当前查看状态并立即发送 Context 2.2 `deferred_grant` 候选；
- 候选通知不访问网关，不携带可兑换 grant；
- 仅在收到前台 Guardian 主动请求后，向网关申请绑定 `mimotrust_guardian_backend` 的一次性 grant；
...
- 通过固定 MethodChannel `com.mimotrust.controlledcontent/context` 将单个
  Context 2.2 JSON 字符串交给 Kotlin；
- Kotlin 校验非空和 32 KB 上限后，使用固定 Action、目标包和 Extra 发送显式广播；
- Kotlin 仅在 `MainActivity` resumed 时动态接收 `REQUEST_CONTENT_CONTEXT`，校验 UUID
  `request_id` 后交给当前活跃 Flutter 内容页；
