...
> 实施现状：沙盒 APK、JSON Schema、样例和代码已迁移到 Context 2.2；双向真机联调待完成
...
沙盒不包含 MiMo、搜索、证据处理、核验任务、核验结果 UI、推荐系统、账号、普通用户
上传和真实社交。允许部署一个与 Android App 分离的开发者内容管理工具，用于规范化上传
演示资源并生成 Manifest；它不属于用户产品界面。
...
内容上架由开发者专用管理服务完成：服务端上传 OSS，统一计算 URL、资源属性和 SHA-256，
生成并校验 Manifest 1.0，再原子更新运行时 registry。普通用户和 Android App 不提供上传
入口；Git 中的固定数据仍作为初始种子和合同夹具。
...
| 检查项 | 结果 |
|---|---|
| Flutter / Dart | Flutter 3.44.2 / Dart 3.12.2；当前五类内容工程已完成构建、分析和测试 |
| ADB | 36.0.0 可用 |
...
1. 在目标真机安装 Context 2.2 沙盒 APK 和 main Guardian；
2. 以一条视频完成候选、主动请求、grant 兑换和任务创建的双向链路；
3. 覆盖超时、重复点击、断网和沙盒后台；
4. 扩展 Guardian/后端的非视频资源处理，再覆盖文章、图文、画廊和音频状态；
5. 归档双向日志、APK 哈希和录屏。
