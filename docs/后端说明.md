...
> 模块化流水线：`backend/app/trust/pipeline_v2/`
...
| 快手 | 专用页面适配器 `backend/app/kuaishou.py` |
| 微博 | 专用页面适配器 `backend/app/weibo.py` |
| 小红书 | 解析 `__INITIAL_STATE__` JSON |
| B站 | yt-dlp 提取 |
| 视频号 | 专用适配器 `backend/app/channels.py` |
| YouTube | yt-dlp 提取 |
...
完整清单见 `backend/.env.example`，关键配置：
