...
def _video_summary_response_format() -> dict[str, Any]:
    return {
        "type": "json_schema",
        "json_schema": {
            "name": "mimo_trust_video_summary_v1",
            "strict": True,
...
""".strip()
    ...
    payload = {
        ...
        "thinking": {"type": "disabled"},
        "response_format": _video_summary_response_format(),
    }
    data = await _completion(payload, timeout=240)
    content, finish_reason = _choice_content(data)
    if finish_reason == "length":
        raise MimoError("视频概括输出达到 token 上限并被截断")
