...
def test_mobile_card_reports_insufficient_evidence_when_analysis_has_critical_gaps() -> None:
    result = SimpleNamespace(
        verification={
            "status": "skipped",
            "message": "当前内容没有需要外部事实核验的现实世界主张。",
        ...
        coverage=SimpleNamespace(
            status="partial",
            critical_gaps=["未配置 MIMO_API_KEY，当前为本地结构化预览"],
        ),
        full_pipeline_milliseconds=9_800,
    )

    card = build_mobile_card("job-partial", result, utc_now())

    assert card.verdict == "证据不足"
    assert card.headline == "当前内容尚未完成充分分析"
    assert "多模态理解" in card.conclusion
    assert card.uncertainty_note == "分析能力或输入覆盖不足，请稍后重试。"
...
def test_android_routes_plain_text_to_agent_context() -> None:
    repository = Path(
        "../guardian/android/app/src/main/java/com/mimotrust/xiaozhen/data/JobRepository.kt"
