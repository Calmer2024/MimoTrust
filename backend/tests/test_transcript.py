...
def test_video_summary_accepts_reasoning_content_and_requests_json_schema(
    monkeypatch,
) -> None:
    calls = []
    ...
    monkeypatch.setattr("app.mimo._completion", fake_completion)
    result = asyncio.run(
        summarize_video("https://example.com/video.mp4", {"title": "标题"})
    )

    assert result["summary"] == "视频讨论新车事故传言。"
    assert calls[0]["response_format"]["type"] == "json_schema"
    assert calls[0]["response_format"]["json_schema"]["strict"] is True
