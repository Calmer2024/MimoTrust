...
def test_android_consumes_and_displays_complete_report_sections() -> None:
    root = Path("../guardian/android/app/src/main/java/com/mimotrust/xiaozhen")
...
def test_sandbox_content_types_flow_into_backend_manifest_analysis() -> None:
    root = Path("../guardian/android/app/src/main/java/com/mimotrust/xiaozhen")
    ...
    backend = Path("app/controlled_content.py").read_text(encoding="utf-8")
    ui = (root / "ui/MimoTrustApp.kt").read_text(encoding="utf-8")
    ...
    assert "suspend fun createControlledContentJob" in repository
    assert '"/v1/content-contexts"' in backend
    assert 'type="controlled_manifest"' in backend
    assert 'job.source.type == "controlled_manifest"' in worker
    assert "analyze_controlled_manifest(job.source.value)" in worker
    assert 'job.source.type in {"shared_url", "platform_api"}' in worker
