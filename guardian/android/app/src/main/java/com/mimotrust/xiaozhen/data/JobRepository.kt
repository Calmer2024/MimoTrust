...
import com.mimotrust.xiaozhen.data.remote.ControlledContentSubmissionDto
...
class JobRepository(
    ...
    suspend fun createControlledContentJob(payload: String, requestId: String): String {
        val contextJson = JsonParser.parseString(payload).asJsonObject
        val response = api.submitContentContext(
            deviceId,
            ControlledContentSubmissionDto(
                requestId = requestId,
                guardianAppVersion = BuildConfig.VERSION_NAME,
                context = contextJson,
            ),
        )
        val content = contextJson.getAsJsonObject("content_ref")
        val contentType = content.get("content_type")?.asString.orEmpty()
        val contentId = content.get("content_id")?.asString.orEmpty()
        ...
        persistQueuedJob(response, "核验 Sandbox $typeLabel：$contentId")
        return response.jobId
    }
